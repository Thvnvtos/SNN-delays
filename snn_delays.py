...
class SnnDelays_onlyLast(Model):
    def __init__(self, config):
        super().__init__(config)
    ...
    def init_model(self):
        ...
        if self.config.init_pos_method == 'uniform':
            ...
            torch.nn.init.uniform_(self.blocks[i][0][0].P, a = self.config.init_pos_a, b = self.config.init_pos_b)
            self.blocks[-1][0][0].clamp_parameters()

            if self.config.model_type == 'snn_delays_lr0':
                self.blocks[-1][0][0].P.requires_grad = False

        
        torch.nn.init.constant_(self.blocks[-1][0][0].SIG, self.config.sigInit)
        self.blocks[-1][0][0].SIG.requires_grad = False
